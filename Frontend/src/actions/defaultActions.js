...
export const getDiagnosis = (body) => async (dispatch) => {
  ...
  try {
    const {
      data
    } = await Axios.post(`${SERVER_API}/Sintomas/enfermedadesRelacionadas`, body);
    dispatch({
...
export const getNearestPlaces = (lat, lng, radius, type, googleApiKey) => async (dispatch) => {
  dispatch({
    type: GET_NEAREST_PLACES_REQUEST
  });
  try {
    const {
      data
    } = await Axios.post(`${SERVER_API}/Maps/getNearestPlaces`, {
      lat,
      lng,
      radius,
      type,
    ...
    dispatch({
      type: GET_NEAREST_PLACES_SUCCESS,
      payload: data.data
    });
  } catch (error) {
    dispatch({
...
export const getDiseasesResults = (disease) => async (dispatch) => {
  dispatch({
    type: GET_DISEASES_RESULTS_REQUEST
  });
  try {
    const {
      data
    } = await Axios.get(`${SERVER_API}/Enfermedad/getEnfermedades?enfermedad=${encodeURIComponent(disease)}`);
    dispatch({
      type: GET_DISEASES_RESULTS_SUCCESS,
      payload: data.data
    });
  } catch (error) {
    dispatch({
...
export const getDiseaseDetails = (disease) => async (dispatch) => {
  dispatch({
    type: GET_DISEASE_DETAILS_REQUEST
  });
  try {
    const {
      data
    } = await Axios.get(`${SERVER_API}/Enfermedad/getEnfermedad?nombre=${encodeURIComponent(disease)}`);
    dispatch({
      type: GET_DISEASE_DETAILS_SUCCESS,
      payload: data.data
    });
  } catch (error) {
    dispatch({
...
export const getFirstAidDetails = (firsAid) => async (dispatch) => {
  dispatch({
    type: GET_FIRST_AID_DETAILS_REQUEST
  });
  try {
    const {
      data
    } = await Axios.get(`${SERVER_API}/Emergencia/get/${encodeURIComponent(firsAid)}`);
    dispatch({
      type: GET_FIRST_AID_DETAILS_SUCCESS,
      payload: data.data
    });
  } catch (error) {
    dispatch({
